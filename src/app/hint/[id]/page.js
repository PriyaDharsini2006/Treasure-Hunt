  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden p-4">
      ...
      <motion.div
        ...
        className="relative z-10 bg-white/5 backdrop-blur-lg border-2 border-[#00f5d0] p-4 md:p-8 rounded-2xl shadow-2xl w-full max-w-md hint-card"
      >
        ...
        {!hints ? (
          ...
          >
            <h2 className="text-2xl md:text-3xl mb-6 md:mb-14 text-center text-[#00f5d0] font-bold flex items-center justify-center gap-3 glitch-text">
              {!hints ? <Lock className="text-[#00f5d0]" size={24} /> : <Unlock className="text-[#00f5d0]" size={24} />}
              Hint Verification
            </h2>
            <div className="flex justify-center mb-4 md:mb-6 transform">
              <img
            ...
            <motion.div className="relative">
              <Key className="absolute left-3 top-1/2 -translate-y-1/2 text-[#00f5d0]" size={16} />
              <input
                ref={emailInputRef}
                type="email"
                value={email}
                ...
                className="w-full bg-transparent pl-8 md:pl-10 pr-3 py-3 md:py-4 border-2 text-[#00f5d0] border-[#00f5d0] rounded-lg focus:outline-none focus:border-white transition duration-300 disabled:opacity-50 text-sm md:text-base"
              />
            </motion.div>
            <motion.button
              type="submit"
              disabled={isVerifying}
              className="w-full bg-[#00f5d0] text-black py-2 md:py-3 rounded-lg hover:bg-gray-400 transition duration-300 flex items-center justify-center gap-2 disabled:opacity-50 glitch-button text-sm md:text-base"
          ...
          >
            ...
            <Sparkles className="mx-auto mb-2 text-[#00f5d0] animate-spin" size={32} />
            ...
            {hints.length > 1 && (
              <div className="flex justify-between mt-3 md:mt-4 space-x-2">
                <motion.button
                  onClick={() => navigateHints('previous')}
                  disabled={currentHintIndex === 0}
                  className="flex items-center gap-1 md:gap-2 text-[#00f5d0] disabled:opacity-50 text-sm md:text-base"
        ...
        {error && (
          <motion.div
            className="text-red-400 mt-2 md:mt-4 text-center glitch-text text-sm md:text-base"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          >
            {error}
